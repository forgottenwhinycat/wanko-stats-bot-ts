...
export async function execute(interaction: ChatInputCommandInteraction) {
        ...
        .composite([
          {
            input: Buffer.from(`
              <svg width="120" height="120">
                ...
                <image href="${u.avatar}" width="120" height="120"
                  clip-path="url(#clip)"
  ...
  <svg width="1500" height="800" xmlns="http://www.w3.org/2000/svg">
    <style>
      .title { fill: #fff; font-size: 60px; font-weight: bold; text-anchor: middle; }
      .name { fill: #fff; font-size: 36px; font-weight: bold; }
      .value { fill: #fff; font-size: 36px; font-weight: bold; text-anchor: end; }
    </style>
  ...
  const composites = [
    ...avatarBuffers.map((buf, i) => {
      const y = rowsY[i];
      return { input: buf, top: y + rowHeight / 2 - 60, left: 170 };
    }),
