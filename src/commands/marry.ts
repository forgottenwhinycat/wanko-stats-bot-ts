...
import { getMarriage, setMarriage, divorceMarriage } from "../firebase/db";
...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(sub =>
    sub
      ...
      .addUserOption(o => o.setName("user").setDescription("Кому пропозиція?").setRequired(true))
  )
  .addSubcommand(sub => sub.setName("accept").setDescription("Прийняти пропозицію одруження"))
  .addSubcommand(sub => sub.setName("decline").setDescription("Відхилити пропозицію одруження"))
  .addSubcommand(sub => sub.setName("divorce").setDescription("Розлучитися з партнером"));
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (sub === "propose") {
    ...
    if (target.id === userId) return interaction.reply({ content: "❌ Ти не можеш одружитися сам із собою!", ephemeral: true });
